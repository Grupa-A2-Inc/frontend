...
export function CourseEditorHeader({
  ...
  return (
    <header className="flex items-center justify-between px-4 py-3 border-b border-brand-primary/10 bg-brand-card flex-shrink-0 gap-2">
      <div className="flex items-center gap-2 min-w-0">
        ...
        >
          ...
          <span className="hidden sm:inline">{mode === "edit" ? "Course" : "My Courses"}</span>
        </Link>
        <span className="text-brand-primary/30 flex-shrink-0 hidden sm:inline">/</span>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {saveErr && <span className="text-red-400 text-xs max-w-[120px] sm:max-w-[200px] truncate">{saveErr}</span>}
        <button
          onClick={onSave}
          disabled={saving}
          className="flex items-center gap-1.5 px-3 sm:px-4 py-2 bg-brand-primary hover:bg-brand-primary/90 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-50"
        >
