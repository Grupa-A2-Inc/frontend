...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 overflow-hidden bg-brand-bg transition-colors duration-300">
      ...
      {/* MOBILE: hero direct, fără animație laptop */}
      <div className="relative z-10 w-full max-w-5xl px-6 flex md:hidden flex-col items-center text-center gap-8 py-10">
        <div className="flex flex-col items-center gap-2">
          <span className="text-xs font-bold tracking-[0.2em] text-brand-primary uppercase bg-brand-primary/10 px-4 py-1.5 rounded-full border border-brand-primary/20">
            AI-powered learning
          </span>
          <h2 className="text-4xl font-black text-white leading-[1.1] tracking-tight mt-2">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-accent">Smart tests.</span>
            <br /> Limitless learning.
          </h2>
          <p className="text-slate-300 text-base font-semibold leading-relaxed max-w-sm">
            Powered by AI — personalized tests, real-time progress, and adaptive learning for everyone.
          </p>
        </div>
        <div className="w-full bg-[#1a2340] rounded-2xl border border-brand-border overflow-hidden shadow-2xl">
          <div className="bg-[#1e1e1e] h-9 flex items-center px-4 gap-2 border-b border-gray-700">
            <div className="w-3 h-3 rounded-full bg-[#ff5f56]" />
            <div className="w-3 h-3 rounded-full bg-[#ffbd2e]" />
            <div className="w-3 h-3 rounded-full bg-[#27c93f]" />
            <span className="ml-2 text-xs text-gray-400 font-mono">ai_test_generator.exe</span>
          </div>
          <div className="p-4 text-left space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-brand-primary/20 border-2 border-brand-primary rounded-lg px-3 py-2 text-white text-xs font-bold flex justify-between items-center">
                <span>B) Generates personalized tests adapted to progress</span>
                <span className="text-brand-primary font-black text-lg ml-2">✓</span>
              </div>
              <div className="bg-[#1e1e1e] border border-gray-600 rounded-lg px-3 py-2 text-gray-400 text-xs font-semibold">C) Requires manual evaluation only</div>
            </div>
          </div>
        </div>
        <div className="flex flex-row justify-center gap-3 w-full">
          {[{ v: "3", l: "core roles" }, { v: "1", l: "unified flow" }, { v: "AI", l: "test gen" }].map((stat) => (
            <div key={stat.l} className="flex-1 bg-brand-accent/10 border border-brand-accent/30 rounded-xl py-3 px-2 flex flex-col gap-1 items-center">
              <span className="text-xl font-black text-brand-accent">{stat.v}</span>
              <span className="text-xs font-bold text-brand-accent/80 uppercase tracking-wider text-center">{stat.l}</span>
            </div>
          ))}
        </div>
      </div>

      {/* DESKTOP: animație laptop */}
      <div className="relative z-10 w-full max-w-5xl px-6 hidden md:block">
