...
export default function UsersPagination({
  ...
  const end = pagination.totalElements === 0
    ? 0
    : Math.min(currentPage * pagination.size + pagination.numberOfElements, pagination.totalElements);
    ...
    <div className="mt-6 flex flex-col gap-3 border-t border-brand-border pt-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3 text-sm text-brand-muted">
        ...
        <select
          ...
          disabled={loading}
          className="rounded-lg border border-brand-border bg-brand-card px-2 py-1.5 text-sm text-brand-text focus:border-brand-primary/60 focus:outline-none disabled:opacity-50"
        >
          {PAGE_SIZES.map((size) => (
